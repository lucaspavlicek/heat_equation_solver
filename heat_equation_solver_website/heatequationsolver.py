...
# turns square tridiagonal matrix into banded form (for scipy.linalg.banded_solve())
def makebanded(A):
    upper = [0.]
    diag = [A[0, 0]]
    lower = []
    for i in range(1, len(A)):
        upper.append(A[i-1, i])
        diag.append(A[i, i])
        lower.append(A[i, i-1])
        
    lower.append(0.)
    return np.array([upper, diag, lower])

# generates a matrix that acts like a Laplacian in 2D (for Crank-Nicolson)
...
# Crank-Nicolson
# Need to utilize banded matrices for this!!!
...
def LOD(Ui, gridsize, iterations, deltat):
    ...
    for i in range(iterations - 1):
        ...
        for j in range(gridsize):
            ...
            Ustar[j] = solve_banded((1, 1), ab, b) #tridiagonal system solver from Scipy
        
        #y direction
        for j in range(gridsize):
            b = (I + (deltat/2)*D2).dot(Ustar[:, j])
            U[i + 1, :, j] = solve_banded((1, 1), ab, b) #tridiagonal system solver from Scipy
...
def ADI(Ui, gridsize, iterations, deltat):
    ...
    for i in range(iterations - 1):
        ...
        for j in range(gridsize):
            b = (I + (deltat/2)*D2).dot(Ustar[j])
            Ustarstar[:, j] = solve_banded((1, 1), ab, b) #tridiagonal system solver from Scipy
...
#this is the function that executes when the run button is clicked
def main(event):
    ...
    Uarray = LOD(Ui, Gridsize, Iterations, Deltat)
    jsU = Uarray.astype(np.uint8).tolist()
